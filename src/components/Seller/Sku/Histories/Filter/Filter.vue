...
export default {
  ...
  computed: {
    ...
    identificationCode() {
      try {
        const identificationCodeObject = this.values.find(element => element.name === 'identification_code');
        return identificationCodeObject.value
      } catch (error) {
        return ''
      }
    },
  ...
  methods: {
    setFilter() {
      ...
      if (this.identificationCode) {
        filter.identification_code = this.identificationCode
      } else if (this.$route.query.identification_code){
        filter.identification_code = null
      }
