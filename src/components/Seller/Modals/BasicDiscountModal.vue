...
<div class="text-center">
    <v-dialog
        v-model="basicDiscountModal.dialog"
        width="908">
        <v-card>
                ...
                <v-col cols="6">
                    <div class="text-right px-5">
                        ...
                        <v-text-field
                            ...
                            v-debounce="calculateBaseDiscount"
                            :rules="rule" />
                    </div>
                </v-col>
...
export default {
    ...
    computed: {
        basicDiscountModal() {
            try {
                return this.$store.getters['get_basicDiscountModal']
            } catch (error) {
                return ''
            }
        },
    ...
    methods: {
        ...
        async getShps() {
            ...
            AxiosMethod.end_point = `seller/${this.$route.params.sellerId}/sku/get/${this.basicDiscountModal?.id}`
        ...
        async updateBaseDiscount() {
            this.loading = true
            var formdata = new FormData();
            const AxiosMethod = new AxiosCall()
            AxiosMethod.end_point = `seller/${this.$route.params.sellerId}/sku/update/base_discount/${this.basicDiscountModal?.id}`
            AxiosMethod.form = formdata

            formdata.append('base_discount', this.discountModel !== null ? this.discountModel : this.data.base_discount)
            formdata.append('base_discount_type', this.discountTypeModel !== null ? this.discountTypeModel : this.data.base_discount_type)
            formdata.append('base_discount_start_time', this.createdAt !== null ? this.createdAt : this.data.base_discount_start_time)
            formdata.append('base_discount_end_time', this.endedAt !== null ? this.endedAt : this.data.base_discount_end_time)
    ...
    created() {
        this.$watch(
            () => this.basicDiscountModal.dialog,
            (dialogState) => {
                if (dialogState) {
                    this.getShps();
                }
