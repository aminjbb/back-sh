    ...
    <v-dialog v-model="dialog" width="468">
        <v-card>
            ...
            <div class="text-center px-5">
                <v-row justify="center" align="center">
                    <v-col cols="11">
                        ...
                        <v-autocomplete variant="outlined" single-line v-model="warehouse" :items="warehouseData" item-title="name" item-value="id" v-debounce="searchWarehouse" />
                    </v-col>
                    <v-col cols="12">
                        <div class="d-flex justify-center d--rtl">
                            <div class=" navbar navbar__scroller" @click="getnavbarItem()">
                                <div class=" d-flex justify-start" id="navbarItems">
                                    <v-card @click="setWorkDay(day)" v-for="(day , index) in  workDays" :key="`workDay${index}`" height="76" min-width="92" elevation="0" class="mx-3 br--8" :class="workDaysClass(day)">
            ...
            <div class="d-flex justify-space-between pb-5 px-10">
                <v-btn width="80" @click="validate()" color="primary500" :loading="loading" height="40" rounded class="px-8 mt-1">
                    تایید
                </v-btn>
                <btn class="mt-3 mr-2" @click="dialog = false" style="cursor: pointer;">
                    انصراف
                </btn>
...
.retailShipment--work-days-card {
    ...
    &__free:hover {
        background: #ffff !important;
        border: 1px solid #110833 !important;
    }
}
</style>
