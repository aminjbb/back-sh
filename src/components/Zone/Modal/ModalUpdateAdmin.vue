    ...
    <v-dialog v-model="dialog" width="580" @input="dialogToggle">
      <v-card>
        ...
        <div class="d-flex justify-center align-center mt-5">
          ...
          <div style="width:80%">
            <v-autocomplete density="compact" variant="outlined"
                            item-title="label"
                            item-value="value"
                            single-line multiple placeholder="انتخاب پیکاپر"
                            :items="adminList" v-model="values">
            </v-autocomplete>
          </div>
        </div>
        ...
        <v-row justify="space-between" align="center" class="pb-4 ma-0">
          <v-col cols="6" class="">
            <div class="text-left">
              <v-btn :loading="loading" @click="addAdmin()" color="primary500" height="40" rounded class="px-5 mt-1 mr-5">
                اعمال تغییرات
              </v-btn>
            </div>
          </v-col>
...
export default {
  setup(){
    const {getAdmins , admin} = new Admin()
  ...
  methods: {
    ...
    async addAdmin() {
      ...
      this.values.forEach((value, index) => {
        formData.append(`admin_ids[${index}]`, value)
      });
