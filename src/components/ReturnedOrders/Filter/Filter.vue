<template>
                ...
                <div>
                    <v-row
                        ...
                        <template v-for="(filter, index) in filterField" :key="index">
                            ...
                            <v-col cols="4" v-if="filter.type === 'select' && filter.value === 'user_id'">
                                <div class="t13300 text-right mb-2">{{filter.name}}</div>
                               <v-autocomplete
                            ...
                            <v-col v-if="filter.type === 'select' && filter.value !== 'user_id'" :cols="filter.value == 'receive_state_id' || filter.value == 'receive_city_id' ? '6' : '4'">
                                <div class="t13300 text-right mb-2">{{filter.name}}</div>
                                <v-select
    ...
    export default {
        ...
        computed: {
            ...
            orderId() {
                try {
                    const idObject = this.values.find(element => element.name === 'order_id');
                    return idObject.value
                } catch (error) {
                    return ''
                }
