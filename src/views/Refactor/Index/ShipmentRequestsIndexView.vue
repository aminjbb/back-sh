                    ...
                    <v-row
                        ...
                        <v-col cols="6">
                            <v-row justify="end">
                                ...
                                <PanelFilter @resetPage="resetPage" path="shipment-requests/index" :filterField="filterField"
                                             :statusItems="status"/>
                            </v-row>
                        </v-col>
                    </v-row>
                ...
                <v-card class="ma-5 mt-0 br-12 flex-grow-1 d-flex flex-column align-stretch" height="580">
                    ...
                    >
                        <template v-slot:customSlot="item">
                            ...
                            <div v-else class="expanded-background" :style="{ backgroundColor: BgSelectedStatus(item.data.custom) }">
                                {{ factorSelectedTitleStatus(item.data.custom) }}
                            </div>
                        </template>
...
export default {
    ...
    methods: {
        ...
        showDropDown(index) {
            const item = this.itemListTable[index];
            if (item.custom === 'in_review') {
        ...
        factorSelectedTitleStatus(status) {
            if (status === 'in_review') {
                return 'در حال بررسی '
            }
            if (status === 'approved') {
        ...
        async updateStatus(status, id) {
            ...
            if (data.status === 'Success') {
                await this.getShipmentRequestsList()
                openToast(
                    this.$store,
    ...
    watch: {
        ...
        ShipmentRequestsList(){
            ...
            this.ShipmentRequestsList.data.forEach((item) =>
                this.itemListTable.push(
                    {
                        id: item.id,
                        type: item.type ? this.translateType(item.type) : 'نامعلوم',
                        shps_count: item.shps_count ? item.shps_count : 'نامعلوم',
                        shps_variety: item.shps_variety ? item.shps_variety : 'نامعلوم',
                        seller: item.seller ? item.seller.shopping_name : item.seller === null ? 'شاواز' : 'نامعلوم',
                        ...
                        updated_at_fa: item.created_at_fa ?  item.updated_at_fa : 'نامعلوم',
                        custom: item.status
                    },
                ),
            )
